...
void extra_mem_calc(particle *p, particle *f[], float *m, int n){
...
void extra_mem_move(particle *p, particle *f, particle *v, float *m, int n, double dt){
    #pragma omp for
    for (int i = 0; i < n; i++){
        particle dv = {
        .x = f[i].x / m[i] * dt,
...
void extra_mem_v(particle *p, particle *f, particle *v, float *m, int n, double *time){
    double ttotal, tinit = 0, tforces = 0, tmove = 0;
    double dt = 1e-5;
    ttotal = omp_get_wtime();
    for (double t = 0; t <= 1; t+= dt){
        tforces -= omp_get_wtime();
        extra_mem_calc(p, &f, m, n);
        tforces += omp_get_wtime();
        #pragma omp barrier
        tmove -= omp_get_wtime();
        extra_mem_move(p, f, v, m, n, dt);
        tmove += omp_get_wtime();
        #pragma omp barrier
    }
    ttotal = omp_get_wtime() - ttotal;
    *time = ttotal;
    printf("# NBody (n=%d)\n", n);
    printf("# Extra memory calculate time (sec): ttotal %.6f, tinit %.6f, tforces %.6f, tmove %.6f\n", ttotal, tinit, tforces, tmove);
}

int main (int argc, char *argv[]) {
    double tinit = 0;
    int n = (argc > 1) ? atoi(argv[1]) : 10;
    char *filename = "extra_mem.csv";
    ...
    for (int i = 0; i < 4; i++) {
        ...
        printf("%d: \n", threads[i]);
        //extra_memory version
        extra_mem_v(p, f, v, m, n, &parallel_time);
